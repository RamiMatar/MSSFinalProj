...
class newMus(torch.utils.data.Dataset):
    def __init__(self, musdb_root, source = 'all', split='train', subset='train', filtered_indices = None, batch_size = None, is_wav=False, sample_rate=44100, segment_length = 10, segment_chunks = 10, discard_low_energy = True, segment_overlap = 0.5, drop_percentile =  0.1, chunks_below_percentile = 0.5):
    ...
    def __getitem__(self, idx):
        ...
        if self.targets != 'all':
            return torch.stack((torch.as_tensor(track.audio, dtype=torch.float32), torch.as_tensor(track.targets[self.targets].audio, dtype=torch.float32)), 0)
        return torch.as_tensor(track.audio, dtype=torch.float32)
