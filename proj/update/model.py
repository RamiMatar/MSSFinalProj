...
class MultiEpochsDataLoader(torch.utils.data.DataLoader):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.batch_sampler = _RepeatSampler(self.batch_sampler, self.batch_size, self.drop_last)
    ...
    def __iter__(self):
        print("CALLED! ", self._DataLoader_initialized)
...
class _RepeatSampler(object):
    ...
    def __iter__(self):
        ...
        print(type(self.sampler.sampler))
        while True:
            batch = []
...
class LightningModel(pl.LightningModule): 
    ...
    def __init__(self, hparams):
        ...
        self.blstms1 = AlternatingBLSTMs(self.K, 24, 63, 64).double()
        self.blstms2 = AlternatingBLSTMs(self.K, 24, 96, 64).double()
        self.blstms3 = AlternatingBLSTMs(self.K, 24, 96, 63 ).double()
    ...
    def train_dataloader(self):
        ...
        trainLoader = MultiEpochsDataLoader(musTraining, batch_size = 8, collate_fn = self.collate, num_workers = 8, shuffle = True, persistent_workers = True)
        return trainLoader

    def val_dataloader(self):
        musValidation = newMus('musdb/', subset = 'train', split= 'valid', batch_size = 16, filtered_indices = self.filtered_validation_indices)
        valLoader = MultiEpochsDataLoader(musValidation, batch_size = 8, collate_fn = self.collate, num_workers = 4, shuffle = False, persistent_workers = True)
        return valLoader
    ...
    def test_dataloader(self):
        if self.testing_dataloader is None:
            ...
            valLoader = MultiEpochsDataLoader(musValidation, batch_size = 1, collate_fn = self.collate, num_workers = 4)
            return valLoader
        else:
